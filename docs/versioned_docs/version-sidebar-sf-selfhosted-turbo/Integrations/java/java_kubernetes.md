...
In this option, the Java application should be run with Jolokia agent and sfKubeAgent running as a sidecar container and fetches metrics via Jolokia port. Refer to [sfKubeAgent](/docs/sidebar-sf-selfhosted-turbo/integrations/kubernetes/sfkubeagent_installation) Overview 
...
Refer to [Prometheus Exporter](/docs/sidebar-sf-selfhosted-turbo/integrations/kubernetes/prometheus_exporter) Overview. Prometheus exporter is deployed as a sidecar container in the application pod and connects to the JMX target exposed by the application to scrape the metrics. sfPod polls Prometheus exporter to scrape the metrics. 
