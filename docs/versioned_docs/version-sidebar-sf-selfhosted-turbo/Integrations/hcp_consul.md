...
SnappyFlow supports monitoring of HCP Consul and Envoy metrics through [StatsD](/docs/sidebar-sf-selfhosted-turbo/Integrations/statsd/custom_monitoring) plugin. The StatsD plugin aggregates all the metrics exposed by HCP Consul and pushes them to SnappyFlow for data visualization and alerting. SnappyFlow provides a built-in dashboard template and a default StatsD rule file, customized to HCP Consul and Envoy metrics to get started easily.

## Pre-Requisites

- Install [sfAgent](/docs/sidebar-sf-selfhosted-turbo/Quick_Start/getting_started#sfagent) on target machine
- Since [StatsD](/docs/sidebar-sf-selfhosted-turbo/Integrations/statsd/custom_monitoring) plugin is used, a custom rules file is required for Monitoring HCP Consul. Default rule files are available at `/opt/sfagent/statsd_rules/` folder.
